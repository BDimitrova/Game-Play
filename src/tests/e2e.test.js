...
describe('E2E tests', function () {
    ...
    describe('Home Page [ 20 Points ]', () => {
        ...
        it('show home page [ 7.5 Points ]', async () => {
            ...
            expect(titles[2]).to.contains("Name_3");
        ...
        it('show details [ 5 Points ]', async () => {
            ...
            await page.click(`.game:has-text("Name_1") >> .details-btn`);
            await page.waitForTimeout(interval);
    ...
    describe('CRUD [ 50 Points ]', () => {
        ...
        it('create makes correct API call for logged in user [ 10 Points ]', async () => {
            ...
            await page.waitForSelector('form');
            await page.fill('[name="leg-title"]', data.title);
            await page.fill('[name="category"]', data.category);
            await page.fill('[name="max-level"]', data.maxLevel);
            await page.fill('[name="game-img"]', data.imageUrl);
            await page.fill('[name="content"]', data.summary);
        ...
        it('edit should populate form with correct data [ 5 Points ]', async () => {
            ...
            expect(inputs[4]).to.contains(data.summary);
        });
        ...
        it('edit does NOT work with empty fields [ 5 Points ]', async () => {
            ...
            await page.fill('[name="leg-title"]', '');
            await page.fill('[name="category"]', '');
            await page.fill('[name="max-level"]', '');
            await page.fill('[name="game-img"]', '');
        ...
        it('edit makes correct API call for logged in user [ 5 Points ]', async () => {
            ...
            await page.fill('[name="leg-title"]', data.title + 'edit');
    ...
    describe('All Games Page for Login Users [ 5 Points ]', async () => {
        ...
        it('check all games page for with 0 games [ 2.5 Points ]', async () => {
            const { get } = await handle(endpoints.catalog);
            get([]);
        ...
        it('check all games page with 2 games [ 2.5 Points ]', async () => {
            const { get } = await handle(endpoints.catalog);
    ...
    describe('All Games Page for Guests [ 5 Points ]', async () => {
    ...
    describe('BONUS: Comments [ 5 Points ]', async () => {

        it('guest can see only comments [ 1 Points ]', async () => {
        ...
        it('guest can\'t add comments [ 1 Points ]', async () => {
            const data = mockData.catalog[1];
            const { get } = await handle(endpoints.delete(data._id));
            get(data);
            await page.goto(host);
            ...
            await page.waitForSelector('#welcome-world');
            await page.click(`.game:has-text("Name_3") >> .details-btn`);
            await page.waitForTimeout(interval);

            expect(await page.isVisible('.create-comment')).to.be.false;
        });

        it('no-author can see field to comments [ 1 Points ]', async () => {
            ...
            expect(await page.isVisible('.create-comment')).to.be.true;
        });

        it('author can\'t see field to comments [ 1 Points ]', async () => {
            const user = mockData.users[0];
            const data = mockData.catalog[0];
            const { get } = await handle(endpoints.details(data._id));
            get(data);
        ...
        it('no-author can post comments [ 1 Points ]', async () => {
            const user = mockData.users[0];
            const data = mockData.catalog[2];
            const { get } = await handle(endpoints.details(data._id));
            get(data);
            ...
            await page.fill('[name="content"]', 'Yeah');
